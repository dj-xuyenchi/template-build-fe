...
export type CallBacks = BaseTable & {
  ...
  handleSetIcon: (row: FeatureDTO, value: string) => void;
  handleSetSortNumber: (row: FeatureDTO, value: number) => void;
  features: FeatureDTO[];
  systems: SystemDTO[];
};
...
export const getColumnsEdit = ({
  ...
  },
  {
    ...
    render: (value: string, record: FeatureDTO, index: number) => (
      <InputNumberCustom
        size="small"
        defaultValue={record.sortNumber}
        onBlur={(e) => {
          const value = e.target.value;
          handleSetSortNumber(record, Number(value));
        }}
      />
    ),
  },
